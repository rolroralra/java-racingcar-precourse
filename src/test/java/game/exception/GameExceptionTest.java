...
class GameExceptionTest {
    ...
    void 예외_생성자_정상_테스트() {
        ...
        racingCarList.forEach(racingCar -> printException(new GameException(GameExceptionCode.RACING_CAR_INVALID_NAME, racingCar)));
        printException(new GameException(GameExceptionCode.GAME_INTERNAL, new RuntimeException("Test Runtime Exception.")));
        printException(new GameException(new RuntimeException("Test Runtime Exception 2")));
    ...
    void 예외_생성자_예외_케이스_테스트() {
        assertThatExceptionOfType(GameException.class).isThrownBy(() -> {
            printException(new GameException(GameExceptionCode.RACING_GAME_INVALID, null));
        });
        assertThatExceptionOfType(GameException.class).isThrownBy(() -> {
            printException(new GameException(GameExceptionCode.RACING_GAME_INVALID, new Object()));
        });
        assertThatExceptionOfType(GameException.class).isThrownBy(() -> {
            printException(new GameException(GameExceptionCode.RACING_GAME_INVALID, racingCarList.get(0)));
        });
    }
